...
void setHadrEffCCxns(BsmParticle &scalar,
    ...
    scalar.setNormalizedCxn(coll, Production::bbHgam, norm(coups.bb),
                            reference);
    scalar.setCxn(coll, Production::pair, EffectiveCouplingCxns::hhggh(coll,
                            m, coups.lam, coups.tt, coups.bb) + 
            EffectiveCouplingCxns::hhvbf(coll,
                            m, coups.lam, coups.tt, coups.bb));
    scalar.setCxn(
        coll, Production::qqH,
...
void setCouplings(BsmParticle &scalar, const NeutralEffectiveCouplings &coups) {
    ...
    scalar.setCoupling(Coupling::alphaCPTauYuk,
                       std::signbit(coups.tautau.real())
                           ...
                           : std::arg(coups.tautau));
    scalar.setCoupling(Coupling::effLam, coups.lam);

    static_assert(magic_enum::enum_count<Coupling>() == 5,
...
// --------------------------  gg -> hh ---------------------------- //
double hhggh(Collider coll, double mH,  double lam, std::complex<double> tt, std::complex<double> bb) {
    static bool warned = false;
    if (coll == Collider::LHC13 &&
        validMassIn<Clamp::lower>(mH, {124., 126.}) &&
        (std::abs(bb) >= 0.9 && std::abs(bb) <= 1.1) &&
        (std::abs(tt) >= 0.9 && std::abs(tt) <= 1.1) &&     
        (std::imag(bb) == 0.0) &&
        (std::imag(tt) == 0.0)) {
// ggH EFT formula from Gudrun et. al. 2304.01968:
        double ttn = std::abs(tt);
        return (0.0722290116102 * std::pow(ttn, 4) + 
               (0.0114281667805 * std::pow(ttn, 2)) * std::pow(lam, 2) +
               (-0.0519952164259 * ttn * lam) * std::pow(ttn, 2));
    } else if (coll == Collider::LHC8) {
        return 0.0;
    } else {
        if (!warned) {
            logger()->warn("Di-higgs non resonant ggF production cross section is not computed. No further warning will be printed.");
            warned = true;
        }
        return 0.0;
    }
...
double hhvbf(Collider coll, double mH,  double lam, std::complex<double> tt, std::complex<double> bb) {
    static bool warned = false;
    if (coll == Collider::LHC13 &&
        validMassIn<Clamp::lower>(mH, {124., 126.}) &&
        (std::abs(bb) >= 0.9 && std::abs(bb) <= 1.1) &&
        ...
        (std::imag(bb) == 0.0) &&
        (std::imag(tt) == 0.0)) {
// Red curve of Fig. 4a 2211.01216 (i.e. ggH + VBF) - ggH EFT formula:
        return (86.43930830760655 - 98.93259229531053 * lam + 
               29.524235848881347 * std::pow(lam, 2) + 0.01414378380077901 * std::pow(lam, 3)
               + 0.0114281667805 * std::pow(lam, 4)) / (18379.008660747746 - 
               5.787367431453021 * lam - 1.0 * 
               std::pow(lam, 2));
    } else if (coll == Collider::LHC8) {
        return 0.0;
    } else {
        if (!warned) {
            logger()->warn("Di-higgs non resonant VBF production cross section is not computed. No further warning will be printed.");
            warned = true;
        }
        return 0.0;
    }
