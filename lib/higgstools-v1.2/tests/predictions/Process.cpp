...
TEST_CASE("PairProductionProcess") {
    ...
    SECTION("default construction") {
        ...
        REQUIRE(proc(pred, h1, h1, 0.5) == 0);
        REQUIRE(proc(pred, h1, h2, 0.5) == 0);
        REQUIRE(proc(pred, h2, h2, 0.5) == 0);
    }
    ...
    SECTION("application to particles") {
        SECTION("process with identical decays, no symmetry factors") {
            ...
            CHECK(
                procSame(pred, h1, h1, 0.5) ==
                Approx(pred.bsmPairCxn(procSame.collider(), h1.id(), h1.id()) *
                       std::pow(h1.br(HP::Decay::bb), 2) * 0.5));
            CHECK(
                procSame(pred, h1, h2, 0.5) ==
                Approx(pred.bsmPairCxn(procSame.collider(), h1.id(), h2.id()) *
                       h1.br(HP::Decay::bb) * h2.br(HP::Decay::bb) * 0.5));
            CHECK(
                procSame(pred, h2, h2, 0.5) ==
                Approx(pred.bsmPairCxn(procSame.collider(), h2.id(), h2.id()) *
                       std::pow(h2.br(HP::Decay::bb), 2) * 0.5));
        }
        ...
        SECTION("process with distinct decays, symmetry factors for identical "
            ...
            CHECK(
                  proc1(pred, h1, h1, 0.75) ==
                  Approx(pred.bsmPairCxn(procSame.collider(), h1.id(), h1.id()) *
                       2 * h1.br(HP::Decay::bb) * h1.br(HP::Decay::gamgam) * 0.75));
            CHECK(
                  proc1(pred, h2, h2, 0.75) ==
                  Approx(pred.bsmPairCxn(procSame.collider(), h2.id(), h2.id()) *
                       2 * h2.br(HP::Decay::bb) * h2.br(HP::Decay::gamgam) * 0.75));
            CHECK(
                proc1(pred, h1, h2, 0.75) ==
                Approx(pred.bsmPairCxn(procSame.collider(), h1.id(), h2.id()) *
                       h1.br(HP::Decay::bb) * h2.br(HP::Decay::gamgam) * 0.75));
        }
        ...
        SECTION("process covering all decay modes, BR(h_i h_j -> anything)=1") {
            ...
            CHECK(procAll(pred, h1, h1, 0.5) ==
                  Approx(h1.cxn(procAll.collider(), HP::Production::pair) * 0.5));
            CHECK(
                procAll(pred, h1, h2, 0.5) ==
                Approx(pred.bsmPairCxn(procAll.collider(), h1.id(), h2.id()) * 0.5));
            CHECK(procAll(pred, h2, h2, 0.5) ==
                  Approx(h2.cxn(procAll.collider(), HP::Production::pair) * 0.5));
        }
            ...
            CHECK(
                  ...
                  Approx(
                       ...
                       (std::pow(h1.br(HP::Decay::bb), 2) +
                            2 * h1.br(HP::Decay::bb) * h1.br(HP::Decay::gamgam) +
                            2 * h1.br(HP::Decay::bb) * h1.br(HP::Decay::tautau) +
                            2 * h1.br(HP::Decay::gamgam) * h1.br(HP::Decay::tautau)) * 0.9));
            CHECK(
                proc(pred, h1, h2, 0.9) ==
                Approx(pred.bsmPairCxn(procSame.collider(), h1.id(), h2.id()) *
                       (h1.br(HP::Decay::bb) * h2.br(HP::Decay::bb) +
                        h1.br(HP::Decay::bb) * h2.br(HP::Decay::gamgam) +
                        h1.br(HP::Decay::tautau) * h2.br(HP::Decay::bb) +
                        h1.br(HP::Decay::tautau) * h2.br(HP::Decay::gamgam))* 0.9));
            CHECK(
                proc(pred, h2, h2, 0.9) ==
                Approx(
                    pred.bsmPairCxn(procSame.collider(), h2.id(), h2.id()) *
                    (std::pow(h2.br(HP::Decay::bb), 2) +
                     2 * h2.br(HP::Decay::bb) * h2.br(HP::Decay::gamgam) +
                     2 * h2.br(HP::Decay::bb) * h2.br(HP::Decay::tautau) +
                     2 * h2.br(HP::Decay::gamgam) * h2.br(HP::Decay::tautau))* 0.9));
        }
    }
    ...
    SECTION("parsing") {
        ...
        CHECK(jproc(pred, h1, h1, 0.5) == proc(pred, h1, h1, 0.5));
    }
}
